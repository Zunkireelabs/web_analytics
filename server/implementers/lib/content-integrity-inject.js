...
// Applies generators/content-integrity-repair.js's five fix shapes against
// the real source file. 'malformed-table'/'raw-text-table'/'duplicate-faq'/
// 'font-size-override' all replace one exact HTML anchor (anchorHtml) with
// new markup (or empty string, for a removal); 'faq-schema-mismatch'
// replaces the exact raw FAQPage <script> text with corrected JSON, same
// shape as schema-repair-inject.js's 'repair-malformed'. Every branch
// anchors on the EXACT text captured at detection time (page-content.js's
// static fetch for the first four; a real Playwright DOM capture for
// font-size-override — see font-consistency-capture.js) — see
// exact-match-patch.js for why that's the only safe way to patch arbitrary
// existing template source; a site that changed since detection (edited the
// table, removed the FAQ, re-rendered from different data, fixed the style
// itself) makes the anchor not-found, and this refuses rather than guessing
// at a new location.
