...
- "component-templates": runs against argv[1], a real checkout of a
  ...
  process) appended to the task as advisory "known issues" text — the same
  RETRIEVE step every other generator gets via server/llm.js's
  withAgentMemory, wired in here since this generator's LLM call never goes
  through callLLM. The derived templates are the deliverable, not any file
  edit — captured from the agent's own final message (conversation.state.events,
  same technique design-agent-poc/run_poc_step4.py validated for extracting a design
