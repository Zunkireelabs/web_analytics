    ...
    <div className="card p-7 relative overflow-hidden">
      <div className="flex items-start justify-between gap-4 flex-wrap mb-5">
        <div>
          <div className="flex items-center gap-2 mb-1.5">
            {forecast?.status === 'ok' && (
              <span className="inline-flex items-center gap-1.5 text-[9.5px] font-extrabold text-indigo-600 bg-indigo-50 border border-indigo-100 rounded-full pl-1.5 pr-2.5 py-1">
      ...
      <div className="flex items-end gap-8 flex-wrap">
        <div>
          ...
          <div className="text-[11px] font-bold text-slate-400 mt-2">Current value</div>
        </div>
        <div className="w-px self-stretch bg-slate-200/70" />
        <div>
        ...
        <div>
          <div className="text-lg font-extrabold" style={{ color: forecast?.confidence != null ? '#6C63FF' : '#94a3b8' }}>
            {forecast?.confidence != null ? `${Math.round(forecast.confidence * 100)}%` : '—'}
          </div>
          <div className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mt-1">Confidence</div>
        </div>
        <div>
          <TrendChip direction={trendDirection} value={wow?.pct_change} />
          <div className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mt-1">Trend, week over week</div>
        </div>
        <div>
          <div className="text-lg font-extrabold text-slate-800">{forecast?.status === 'ok' ? forecast.model : '—'}</div>
          <div className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mt-1">Model in use</div>
        </div>
        {horizonPctChange != null && (
          <div>
            <TrendChip direction={horizonPctChange > 0 ? 'up' : 'down'} value={horizonPctChange} />
            <div className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mt-1">Projected change</div>
          </div>
        )}
      </div>
    </div>
