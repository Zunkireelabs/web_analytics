...
export default function LiveActivityRail({ items }) {
  return (
    <div className="card overflow-hidden flex flex-col" style={{ maxHeight: 260 }}>
      <div className="flex items-center gap-2 px-4 py-3 shrink-0" style={{ borderBottom: '1px solid rgba(15,23,42,0.08)' }}>
        ...
        <p className="text-[11px] font-bold uppercase tracking-widest text-slate-500">Live Activity</p>
      </div>
        ...
        {items === null ? (
          <div className="p-4 space-y-2.5">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="h-8 rounded-lg animate-pulse" style={{ background: 'rgba(15,23,42,0.06)' }} />
            ))}
          </div>
        ) : items.length === 0 ? (
          <p className="text-[12px] text-slate-400 text-center py-8 px-4">No agent activity yet.</p>
        ) : (
          <div>
            {items.map((a, i) => {
              ...
              return (
                <div key={i} className="flex items-center gap-2.5 px-4 py-2" style={{ borderTop: i ? '1px solid rgba(15,23,42,0.06)' : 'none' }}>
                  <span className="w-6 h-6 rounded-lg grid place-items-center text-[11px] shrink-0"
                    style={failed ? { background: 'rgba(15,23,42,0.06)', color: '#64748b' } : { background: `${cat.color}26`, color: cat.color }}>
                    {cat.icon}
                  </span>
                  <span className={`text-[11.5px] flex-1 min-w-0 truncate ${failed ? 'text-slate-400 italic' : 'text-slate-700'}`}>
                    {failed ? `Attempted: ${a.label.charAt(0).toLowerCase() + a.label.slice(1)}` : a.label}
                  </span>
                  <span className="text-[10px] text-slate-400 font-mono shrink-0">{timeAgo(a.createdAt)}</span>
                </div>
              );
            })}
          </div>
