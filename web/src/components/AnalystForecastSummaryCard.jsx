  ...
  return (
    <div className="an-panel p-6 flex flex-col">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="text-xs font-black uppercase tracking-wider text-slate-400">Forecast Summary</h3>
      </div>

      {forecast?.status !== 'ok' ? (
        <p className="text-xs font-medium text-slate-500">No forecast available for this metric yet.</p>
      ) : (
        <>
          ...
          <div className="flex items-center gap-4 mt-5">
            <svg width="66" height="66" viewBox="0 0 72 72">
              ...
              />
              <text x="36" y="40" textAnchor="middle" fontSize="14" fontWeight="800" fill="#f8fafc">
                {confidence != null ? `${Math.round(confidence * 100)}%` : '—'}
              </text>
            </svg>
            <div>
              <div className="text-base font-extrabold" style={{ color: reliability.color }}>{reliability.label}</div>
              <div className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Forecast reliability</div>
            </div>
          </div>

          <div className="mt-4 divide-y divide-slate-700/50 border-t border-slate-200">
