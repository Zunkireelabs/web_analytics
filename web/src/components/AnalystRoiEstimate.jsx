...
export default function AnalystRoiEstimate({ result, error }) {
  if (error) return <p className="text-[10px] font-semibold text-rose-500">{error}</p>;
    ...
    return (
      <p className="text-[10px] font-bold text-slate-700">
        ...
        {result.confidence != null && (
          <span className="text-slate-400 font-semibold"> · {Math.round(result.confidence * 100)}% confidence</span>
        )}
      </p>
    );
  }
  ...
  return <p className="text-[10px] font-semibold text-slate-400">Not computable for this finding.</p>;
}
