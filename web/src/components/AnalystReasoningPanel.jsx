...
export default function AnalystReasoningPanel({ clientId, insight, metric }) {
  ...
  if (!insight) {
    return (
      <div className="card p-6">
        <PanelHeader />
        <p className="text-xs text-slate-400 font-medium mt-3">Select a finding to see how the AI traced its cause.</p>
      </div>
    );
  }
  ...
  return (
    <div className="card p-6">
      <PanelHeader />
      {state === null && <p className="text-xs text-slate-400 font-medium animate-pulse mt-3">Loading root cause…</p>}
      {data?.status === 'unavailable' && <p className="text-xs text-slate-400 font-medium mt-3">This finding has no id to look up.</p>}
      {data?.status === 'not-yet-computed' && (
        <p className="text-xs text-slate-400 font-medium mt-3">Root cause analysis runs nightly — check back after the next run.</p>
      )}
      {(data?.status === 'insufficient-data' || data?.status === 'error') && (
        <p className="text-xs text-slate-400 font-medium mt-3">{data.error || 'Not enough data to attribute a root cause for this finding.'}</p>
      )}
      {data?.status === 'ok' && (
        <div className="mt-4 flex flex-col items-start gap-1">
          <ChainNode label="Root Metric Change" node={data.root} unit={metric?.unit} highlight />
          <ArrowDown size={14} className="text-slate-300 ml-4" />
...
function PanelHeader() {
  return (
    <div className="flex items-center gap-2">
      ...
      <h3 className="text-xs font-black uppercase tracking-wider text-slate-600">AI Reasoning Panel</h3>
    </div>
  );
}
...
function ChainNode({ label, node, unit, highlight }) {
  ...
  return (
    <div className={`rounded-2xl border p-3 min-w-[160px] ${highlight ? 'border-violet-200 bg-violet-50/60' : 'border-slate-150 bg-slate-50/60'}`}>
      ...
      {node.share_of_baseline_change_pct != null && (
        <div className="text-[9px] font-bold text-slate-500 mt-1">{Math.round(node.share_of_baseline_change_pct)}% of baseline change</div>
      )}
    </div>
  );
}
