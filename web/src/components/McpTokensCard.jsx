...
import { KeyRound, Copy, Check, Ban, Info, Lock } from 'lucide-react';
  ...
  return (
    <div className="bg-white/70 border border-slate-200/50 backdrop-blur-md shadow-sm rounded-2xl p-6">
      ...
      <p className="text-xs text-slate-450 font-semibold mb-4">
        ...
        Each token can only do what its permission level allows.
      </p>
      ...
      {justCreated && (
        <div className="bg-emerald-50 border border-emerald-100 rounded-xl px-4 py-3.5 mb-5 space-y-2.5">
...
{connectSnippet(justCreated.token)}
      ...
      {!confirmingPassword ? (
        <form onSubmit={submit} className="mb-5 max-w-md space-y-4">
        ...
        <form onSubmit={confirmPasswordAndCreate} className="mb-5 max-w-md space-y-3 bg-amber-50 border border-amber-100 rounded-xl p-4">
      ...
      {!tokens ? (
        ...
        <div className="space-y-2">
          {tokens.map((t) => (
            <div key={t.id} className="flex items-center justify-between gap-3 border border-slate-100 rounded-xl px-3.5 py-2.5">
              <div className="min-w-0">
                <p className="text-xs font-bold text-slate-800 truncate flex items-center gap-2 flex-wrap">
                  ...
                  <span className="text-[9px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded-md bg-slate-100 text-slate-500">
                    {TIERS.find((tier) => tier.value === t.permission_level)?.label || t.permission_level}
                  </span>
                </p>
