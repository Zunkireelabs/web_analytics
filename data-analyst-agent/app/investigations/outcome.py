...
Two evaluation paths, covering every investigation a human actually approved
(status == 'approved' — see app/api/routes/investigations.py's approve
endpoint), not just forecast_risk:

- forecast_risk (_evaluate_one): a real prediction exists (the investigation's
  captured forecast_outlook — predicted_date/last_actual/pct_projected_change,
  literally the triggering Insight's own evidence, see
  app/investigations/engine.py), so the outcome compares that prediction
  against the real actual once predicted_date has passed.
- trend_shift / anomaly / milestone (_evaluate_observed_decline_one): these
  insight_types have no prediction to check — they observe a decline that
  ALREADY happened — so the honest question is different: did the metric
  recover after a human approved the investigation, or keep declining?
  Baseline is the real value captured in the insight's own evidence at
  investigation time (evidence.current_value / evidence.value), actual is the
  first real MetricObservation on or after OBSERVED_EVALUATION_HORIZON_DAYS
  past approval — never a sampled or estimated value, same discipline as the
  forecast_risk path. content_decay/cannibalization/target_keyword_evidence
  are deliberately NOT included here: their evidence carries no single
  before-value field (content_decay is 3 weeks of pct_change, not a level),
  so there is nothing honest to set a baseline from without inventing one.

Every input to both paths is a row an earlier stage already wrote, compared
against a real MetricObservation, never a sampled or estimated actual — same
discipline as app/forecast/accuracy.py, which this module deliberately does
not duplicate: that one tracks raw forecast-point accuracy for every forecast
...
# How long to wait after a human approves a trend_shift/anomaly/milestone
# investigation before checking whether the metric recovered — long enough
# for a shipped fix to plausibly show up in GSC/GA4 reporting (GSC in
# particular lags real-world changes by days), short enough that "did this
# help" is still answered on a useful timescale rather than never.
OBSERVED_EVALUATION_HORIZON_DAYS = 30

# Within this band, the metric is read as flat rather than meaningfully
# recovered or worsened — avoids a coin-flip 'improved'/'worsened' label on
# ordinary day-to-day noise around the baseline.
OBSERVED_FLAT_BAND_PCT = 3.0

OBSERVED_DECLINE_INSIGHT_TYPES = ("trend_shift", "anomaly", "milestone")
...
async def _evaluate_client(session: AsyncSession, client_id: int) -> None:
    ...
    candidates = (
        await session.execute(
            select(Investigation).where(
                ...
                Investigation.insight_type.in_(("forecast_risk", *OBSERVED_DECLINE_INSIGHT_TYPES)),
            )
        )
    ).scalars().all()
    ...
    for inv in candidates:
        if inv.id in already_evaluated:
            continue
        try:
            if inv.insight_type == "forecast_risk":
                await _evaluate_one(session, client_id, inv)
            else:
                await _evaluate_observed_decline_one(session, client_id, inv)
...
async def _evaluate_observed_decline_one(session: AsyncSession, client_id: int, inv: Investigation) -> None:
    baseline_raw = inv.evidence.get("current_value") if inv.evidence else None
    if baseline_raw is None and inv.evidence:
        baseline_raw = inv.evidence.get("value")  # anomaly's evidence key
    if baseline_raw is None:
        return  # this insight_type's evidence carries no real before-value — never invent one
    ...
    approved_at = await session.scalar(
        select(InvestigationEvent.created_at).where(
            InvestigationEvent.investigation_id == inv.id, InvestigationEvent.to_status == "approved",
        ).order_by(InvestigationEvent.created_at.desc()).limit(1)
    )
    if approved_at is None:
        return  # status is 'approved' but no event recorded the transition — nothing to measure from

    horizon_date = approved_at.date() + timedelta(days=OBSERVED_EVALUATION_HORIZON_DAYS)
    if horizon_date > date.today():
        return  # too early — retry a later night once the horizon has actually passed

    actual = await session.scalar(
        select(MetricObservation.value).where(
            MetricObservation.client_id == client_id, MetricObservation.metric_key == inv.metric_key,
            MetricObservation.dimension_type == inv.dimension_type, MetricObservation.dimension_value == inv.dimension_value,
            MetricObservation.period_start >= horizon_date,
        ).order_by(MetricObservation.period_start.asc()).limit(1)
    )
    if actual is None:
        return  # no real observation has landed at or past the horizon yet — retry a later night

    actual_value = float(actual)
    pct_actual_change = (actual_value - baseline_value) / baseline_value * 100

    if pct_actual_change > OBSERVED_FLAT_BAND_PCT:
        outcome_status = "improved"
    elif pct_actual_change < -OBSERVED_FLAT_BAND_PCT:
    ...
    session.add(InvestigationOutcome(
        client_id=client_id, investigation_id=inv.id,
        baseline_value=baseline_value, predicted_value=None,
        actual_value=actual_value, pct_projected_change=None, pct_actual_change=pct_actual_change,
        outcome_status=outcome_status,
    ))
    session.add(InvestigationEvent(
        investigation_id=inv.id, from_status=inv.status, to_status="completed",
        actor="system", detail={"outcome_status": outcome_status, "pct_actual_change": pct_actual_change},
    ))
    inv.status = "completed"
...
async def get_investigation_outcome_reliability(session: AsyncSession, client_id: int) -> dict:
    ...
    Scoped to forecast_risk rows only (predicted_value IS NOT NULL — see
    migration 0039) — an 'improved'/'unchanged'/'worsened' observed-decline
    outcome answers "did the metric recover", not "was the forecast right",
    and mixing the two in would silently corrupt this forecast-confidence
    signal with unrelated data."""
    statuses = (
        await session.execute(
            select(InvestigationOutcome.outcome_status).where(
                InvestigationOutcome.client_id == client_id, InvestigationOutcome.predicted_value.is_not(None),
            )
        )
    ).scalars().all()
