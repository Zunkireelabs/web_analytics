...
Prioritizer -> Predictive Alert Delivery. Each stage reads only what the
previous stage wrote (alert delivery reads the Recommendation text the
prior stage just wrote, when available). Feature Importance runs after
Forecasts but before Insights — it's descriptive of a metric's whole
history, independent of that night's insights, but early enough that a
later stage could cite it. Root Cause Analysis runs right after Insights,
since it's triggered BY that night's anomaly/trend_shift insights, and
before Recommendations so a future recommendation pass could cite it too.
Effort Estimation and Time-to-Impact Prediction both run after
...
summary, the direct API route). The Recommendation Prioritizer runs last,
before alert delivery, since its formula consumes Opportunity Scoring's and
Effort Estimation's output.
